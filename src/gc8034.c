...
static int gc8034_get_frame_interval(struct v4l2_subdev *sd,
	struct v4l2_subdev_state *state,
...
static int gc8034_apply_otp(struct gc8034 *gc8034)
{
	...
	if (otp_ptr->flag & 0x04) {
		...
		dev_dbg(&client->dev, "apply chip reg\n");
	}
	return 0;
}
...
static int gc8034_get_mbus_config(struct v4l2_subdev *sd, unsigned int pad,
				  struct v4l2_mbus_config *config)
{
	...
	if (2 == sensor->lane_num) {
		...
		config->link_freq = GC8034_MIPI_FREQ_336MHZ;
...
static const struct v4l2_subdev_pad_ops gc8034_pad_ops = {
	...
	.get_frame_interval = gc8034_get_frame_interval,
	.get_fmt = gc8034_get_fmt,
	.set_fmt = gc8034_set_fmt,
	.get_mbus_config = gc8034_get_mbus_config,
};
...
static int gc8034_parse_of(struct gc8034 *gc8034)
{
	...
	if (!dev->of_node) {
		...
		dev_info(dev, "using ACPI defaults: lane_num(%d) pixel_rate(%u)\n",
...
static int gc8034_probe(struct i2c_client *client)
{
	...
	if (node) {
		...
		gc8034->module_index = 1;
		gc8034->module_facing = "front";
		gc8034->module_name = "gcti8034";
		gc8034->len_name = "default";
	}
	gc8034->client = client;

	gc8034->xvclk = devm_clk_get_optional(dev, "xvclk");
	if (IS_ERR(gc8034->xvclk)) {
		dev_err(dev, "Failed to get xvclk\n");
		return PTR_ERR(gc8034->xvclk);
	}
...
static const struct acpi_device_id gc8034_acpi_match[] = {
	{ "GCTI8034" },
	{ },
};
MODULE_DEVICE_TABLE(acpi, gc8034_acpi_match);
...
MODULE_DEVICE_TABLE(i2c, gc8034_match_id);
...
static struct i2c_driver gc8034_i2c_driver = {
	.driver = {
		...
		.acpi_match_table = gc8034_acpi_match,
	},
	.probe		= &gc8034_probe,
