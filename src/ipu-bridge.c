						 ...
						 struct ipu_sensor *sensor)
{
	...
	if (sensor->vcm_type)
		snprintf(sensor->node_names.vcm, sizeof(sensor->node_names.vcm),
			 "%.10s%c%u", sensor->vcm_type,
			 bridge->ipu_node_name[6], sensor->link);
	...
	if (sensor->csi_dev) {
		...
		snprintf(sensor->ivsc_name, sizeof(sensor->ivsc_name),
			 "%.7s%c%u", device_hid, bridge->ipu_node_name[6],
			 sensor->link);
				     ...
				     struct ipu_bridge *bridge)
{
	...
	for_each_acpi_dev_match(adev, cfg->hid, NULL, -1) {
		...
		snprintf(sensor->name, sizeof(sensor->name), "%.7s%c%u",
			 cfg->hid, bridge->ipu_node_name[6], sensor->link);
...
{
	static const char * const ipu_node_names[] = {
		IPU_HID,
		"INT343F",
		"INT343G",
		"INT343H",
	...
	/*
	 * Some systems keep secondary software fwnodes around across module
	 * reloads. Always rebuild the bridge graph so newly-supported ACPI HIDs
	 * are considered after an updated module is installed.
	 */
	...
	for (i = 0; i < ARRAY_SIZE(ipu_node_names); i++) {
		strscpy(bridge->ipu_node_name, ipu_node_names[i],
