...
/*
 * gc5035 / gti5035-compatible driver
 *
...
static int gc5035_get_frame_interval(struct v4l2_subdev *sd,
	struct v4l2_subdev_state *state,
...
static int sensor_get_mbus_config(struct v4l2_subdev *sd, unsigned int pad,
				  struct v4l2_mbus_config *config)
{
	...
	if (2 == sensor->lane_num) {
		...
		config->link_freq = GC5035_LINK_FREQ_MHZ;
...
static const struct v4l2_subdev_pad_ops gc5035_pad_ops = {
	...
	.get_frame_interval = gc5035_get_frame_interval,
...
static int gc5035_initialize_controls(struct gc5035 *gc5035)
{
	...
	orientation = V4L2_CAMERA_ORIENTATION_FRONT;
	if (!strcmp(gc5035->module_facing, "back"))
		orientation = V4L2_CAMERA_ORIENTATION_BACK;
	ctrl = v4l2_ctrl_new_std_menu(handler, NULL,
	...
	ctrl = v4l2_ctrl_new_std(handler, NULL,
...
static int gc5035_parse_of(struct gc5035 *gc5035)
{
	...
	if (!endpoint) {
		dev_info(dev, "no firmware endpoint, using default 2-lane CSI-2 config\n");
		gc5035->lane_num = 2;
...
static int gc5035_probe(struct i2c_client *client)
{
	...
	gc5035->module_index = 0;
	gc5035->module_facing = "back";
	gc5035->module_name = GTI5035_NAME;
	gc5035->len_name = "default";
	if (node) {
		of_property_read_u32(node, RKMODULE_CAMERA_MODULE_INDEX,
			&gc5035->module_index);
		of_property_read_string(node, RKMODULE_CAMERA_MODULE_FACING,
			&gc5035->module_facing);
		of_property_read_string(node, RKMODULE_CAMERA_MODULE_NAME,
			&gc5035->module_name);
		of_property_read_string(node, RKMODULE_CAMERA_LENS_NAME,
			&gc5035->len_name);
	}
	gc5035->client = client;

	gc5035->xvclk = devm_clk_get_optional(dev, "xvclk");
	if (IS_ERR(gc5035->xvclk)) {
		dev_err(dev, "Failed to get xvclk\n");
		return PTR_ERR(gc5035->xvclk);
	}
...
static const struct of_device_id gc5035_of_match[] = {
	...
	{ .compatible = "gti,gti5035" },
	{},
};
MODULE_DEVICE_TABLE(of, gc5035_of_match);
#endif

static const struct acpi_device_id gc5035_acpi_match[] = {
	{ "GCTI5035" },
	{ }
};
MODULE_DEVICE_TABLE(acpi, gc5035_acpi_match);
...
static struct i2c_driver gc5035_i2c_driver = {
	.driver = {
		...
		.acpi_match_table = gc5035_acpi_match,
	},
	.probe		= &gc5035_probe,
...
MODULE_ALIAS("i2c:gti5035");
MODULE_DESCRIPTION("GalaxyCore gc5035 / GTI5035-compatible sensor driver");
